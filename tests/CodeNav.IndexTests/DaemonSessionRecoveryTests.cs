...
public sealed class DaemonSessionRecoveryTests
{
    private static DaemonUnavailableFailure Retryable => new("daemon_handshake_timeout", "timeout", "retry", true);
    ...
    [InlineData("initialize-protocol-error")]
    public async Task DispatchedRecoveryFailuresNeverReplayAndNextCallUsesAWorkingConnection(string failure)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                Assert.Equal(initializeFailure ? "daemon_connection_unavailable" : "daemon_request_outcome_unknown", Cause(lost));
                Assert.Equal(initializeFailure, lost.StructuredContent!.Value.GetProperty("retryable").GetBoolean());
            }
        ...
        async Task ServeAsync(NetworkStream stream, string? injectedFailure)
        {
            ...
            while (await reader.ReadLineAsync(lifetime.Token) is { } line)
            {
                ...
                else
                {
                    ...
                    if (root.GetProperty("params").GetProperty("name").GetString() == "mutate")
                    {
    ...
    public async Task ConcurrentRecoverySharesAttemptAndCancelledWaiterDoesNotCancelPeer()
    {
        ...
        await using var recovery = new DaemonSessionRecovery(Retryable, async token =>
        {
            ...
            throw new DaemonProxyFailureException(attempt == 1 ? Retryable :
                new("daemon_response_authority_failed", "refused", "inspect", false));
        }, CancellationToken.None);
        using var cancelled = new CancellationTokenSource();
        var first = recovery.InvokeAsync(Request(server), cancelled.Token).AsTask();
        await entered.Task.WaitAsync(TimeSpan.FromSeconds(5));
        var peer = recovery.InvokeAsync(Request(server), CancellationToken.None).AsTask();
        try
        {
        ...
        Assert.Equal("daemon_handshake_timeout", Cause(await peer));
        Assert.Equal("daemon_response_authority_failed", Cause(await recovery.InvokeAsync(Request(server), CancellationToken.None)));
