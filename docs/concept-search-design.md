...
opening files, and trying again. In a repository with thousands of projects this has predictable
costs:
...
search_concept(
  ...
  queryScope: "default" | "all" | "first_party" = "default",
  project: string? = null,
...
- filtered-query behavior across thousands of projects;
...
| Base-indexed vendored path | Eligible if present, marked `noise`, demoted or removed by `queryScope:"first_party"` |
...
- Run full performance, soak, branch-switch, and corruption tests across thousands of projects and
  14M lines.
