...
- **Reuse phoenix's semantic core; do not rebuild Roslyn project-loading.** The dataflow analysis
  ...
  `references`. A separate process re-loading thousands of csproj files is wasteful and creates
  two freshness models. Lean: dispatch-map as a **capability/tool-group sharing phoenix's core
  library**, not a second workspace. The `impls`-fold-in reinforces this.
