...
public sealed class IndexStore : IDisposable
{
    ...
    private void ExecuteRawInsert(sqlite3_stmt statement, string rowKind)
    {
        ...
        // Every parameter is rebound before the next step, so reset is sufficient; clearing every
        // binding would add another native call per parameter without changing semantics.
