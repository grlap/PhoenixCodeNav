...
public sealed partial class SemanticService
{
        ...
        IReadOnlyCollection<FSharpPackageReferenceSnapshot> packageReferences)
    {
        ...
        {
            if (!TryGetFSharpAssetsProperty(dependencies, reference.Id,
                    out JsonElement dependency) ||
                dependency.ValueKind != JsonValueKind.Object ||
                !dependency.TryGetProperty("target", out JsonElement dependencyTarget) ||
                dependencyTarget.ValueKind != JsonValueKind.String ||
                !dependencyTarget.GetString()!.Equals("Package",
                    StringComparison.OrdinalIgnoreCase))
                return false;
    ...
    private static bool TryGetFSharpAssetsProperty(JsonElement value, string name,
        out JsonElement propertyValue)
    {
        if (value.TryGetProperty(name, out propertyValue)) return true;
        foreach (JsonProperty property in value.EnumerateObject())
        {
