...
internal static class Json
{
    ...
    /// <summary>Five-list variant used when a response must preserve aggregate counts while
    /// independently trimming diagnostics, contexts, coverage details, groups, and finally the
    /// primary results.</summary>
    public static string WithAuxiliaryListsBudget<T, TAux, TSecondary, TTertiary, TQuaternary>(
        List<T> items,
        List<TAux> auxiliary,
        List<TSecondary> secondary,
        List<TTertiary> tertiary,
        List<TQuaternary> quaternary,
        Func<List<T>, bool, List<TAux>, bool, List<TSecondary>, bool,
            List<TTertiary>, bool, List<TQuaternary>, bool, object> build,
        int? maxBytes = null)
    {
        int cap = Math.Min(maxBytes ?? HardBudgetBytes, HardBudgetBytes);
        var work = new List<T>(items);
        ...
        var secondaryWork = new List<TSecondary>(secondary);
        var tertiaryWork = new List<TTertiary>(tertiary);
        var quaternaryWork = new List<TQuaternary>(quaternary);
        bool truncated = false;
        bool auxiliaryTruncated = false;
        bool secondaryTruncated = false;
        bool tertiaryTruncated = false;
        bool quaternaryTruncated = false;
        string json = Serialize(build(work, truncated, auxiliaryWork, auxiliaryTruncated,
            secondaryWork, secondaryTruncated, tertiaryWork, tertiaryTruncated,
            quaternaryWork, quaternaryTruncated));

        while (Utf8Bytes(json) > cap && quaternaryWork.Count > 0)
        {
            int keep = quaternaryWork.Count / 2;
            quaternaryWork.RemoveRange(keep, quaternaryWork.Count - keep);
            quaternaryTruncated = true;
            json = Serialize(build(work, truncated, auxiliaryWork, auxiliaryTruncated,
                secondaryWork, secondaryTruncated, tertiaryWork, tertiaryTruncated,
                quaternaryWork, quaternaryTruncated));
        }
