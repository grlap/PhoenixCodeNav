    ...
    public string ServerCapabilities()
    {
        ...
        return Json.Serialize(new
        {
            ...
            features = new object[]
            {
                ...
                new { id = "hierarchy-ranking", summary = "implementations ranked concrete-first with derivation 'via' + a likelyImplementation flag. By design these appear CONDITIONALLY: likelyImplementation only when exactly ONE concrete implementation exists; 'via' only when a type implements the queried interface INDIRECTLY through a base — a flat N-implementer set legitimately shows neither. Example shapes when they DO fire: {implementations:[{symbol:{...SqlWidgetStore}, rank:'concrete'}], concreteCount:1, likelyImplementation:'SqlWidgetStore'} and {symbol:{...CachedStore}, rank:'concrete', via:'StoreBase'} (CachedStore : StoreBase : IWidgetStore). Envelope note: implementations wraps hits in {symbol, rank, via}; type_hierarchy.derivedOrImplementing returns flat symbol objects — a historical drift, documented rather than broken; pin shapes per tool" },
                ...
                new { id = "test-classification", summary = "isTest is REFERENCE-driven: test frameworks via packages OR binary <Reference> (nunit/xunit/MSTest, incl. non-standard names containing nunit.framework), plus compiled-[TestFixture]-attributes-on-graph-leaf promotion for custom-resolve builds where the framework never appears in the csproj. Name shapes are a narrow dotted-suffix fallback only — TestRoute-style names never classify. NAME-uniform across same-AssemblyName csproj pairs. Release note: classification broadened in schema v7 (reference signals) — a monolith-scale deploy reclassified ~100 legacy test projects, so isTest state cached from earlier schemas may differ" },
