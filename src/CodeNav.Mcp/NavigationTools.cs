        ...
        bool includeFeatureSummaries = false, string defaultQueryScope = "all")
    {
        ...
        object envelope = new
        {
            ...
            features = new object[]
            {
                ...
                new { id = "fsharp-unsupported-language-boundary", summary = "F# callers/callees, implementations, hierarchy, C#-targeted ProjectReference semantics, multi-target compatible selection, and netstandard1.x compile-input materialization remain unsupported; indexed F# symbol-name search, F# ProjectReference definition closure with exact or table-authorized single-target netstandard2.0/2.1 children, and selected-project compiler-bound references are supported" },
            ...
            semantic = new
            {
                ...
                note = "C# exact results are scoped to loaded candidate clusters. F# symbol_at and definition are compiler-checked from one proven physical root project/TFM through its F# ProjectReference closure; exact child TFMs win, while v0.12.84 admits only table-authorized single-target netstandard children and materializes 2.0/2.1 end to end. Multi-target compatibility and netstandard1.x compile inputs fail closed. SDK projects are transitive by default while DisableTransitiveProjectReferences=true and legacy projects are direct-only, and child compiler errors retain source provenance and downgrade confidence like root errors. References use the closure but count only the selected root and remain a workspace lower bound because dependents are not scanned; successful results are exact only while disclosed partial reasons preserve selected-context authority; every error, authority loss, or unclassified partial reason is indexed. Restored package compile assets are snapshotted; C# dependencies fail closed without borrowing last-built DLLs; partial fields report bounded authority.",
