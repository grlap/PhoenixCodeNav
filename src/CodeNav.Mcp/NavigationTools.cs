...
public sealed partial class NavigationTools
{
        ...
        bool includeFeatureSummaries = false, string defaultQueryScope = "all")
    {
        ...
        object envelope = new
        {
            ...
            features = new object[]
            {
                ...
                new { id = "fsharp-symbol-at-semantic", summary = "v0.12.5 FCS position resolution for one explicit physical .fsproj + TFM type-check context; since v0.12.83 the context includes its exact-TFM F# ProjectReference closure under the evaluated MSBuild transitivity policy" },
                new { id = "fsharp-definition-same-project", summary = "v0.12.5 FCS signature/implementation declarations for the selected physical F# project; since v0.12.83 position resolution can return declarations from its exact-TFM F# ProjectReference closure under the evaluated MSBuild transitivity policy" },
                ...
                new { id = "fsharp-references-same-project", summary = "v0.12.81 references position mode counts compiler-bound non-definition uses in one selected physical .fsproj + TFM, returns bounded samples from the pinned source snapshot for that root, and reports the selected-project count as a workspace lower bound because dependent projects are not scanned; since v0.12.83 FCS binds that root through its matching-TFM F# ProjectReference closure under the evaluated MSBuild transitivity policy without counting dependency-project uses" },
                new { id = "fsharp-semantic-project-reference-closure", summary = "v0.12.83 symbol_at and definition recursively type-check active F# ProjectReference inputs from one pinned index snapshot at the exact selected TFM; literal physical project paths select legacy/.Net companions; FCS receives dependency-first in-memory referenced-project options without emitted or last-built DLLs, using MSBuild's flat transitive SDK default while DisableTransitiveProjectReferences=true and legacy-style projects remain direct-only; each physical node is checked once and child compiler errors retain their source paths and downgrade confidence through fsharp_semantic_diagnostics_present; returned dependency declarations are attributed by declarationsFromProjectReferenceClosureCount while declarationsOutsideSelectedProjectCount retains its not-returned meaning; references still count only the selected root; missing, non-F#, cyclic, metadata-unsupported, same-assembly, or exact-TFM-unavailable closure fails closed" },
                ...
                new { id = "fsharp-semantic-bounded-project-evaluation", summary = "v0.12.6 bounded simple properties before semantic items, conditions, Choose, and workspace-local .props imports feed FCS; active ProjectReference identities use bounded literal metadata since v0.12.83; unresolved SDK/toolchain/ambient inputs remain explicit; no targets/tasks or property functions" },
                ...
                new { id = "fsharp-unsupported-language-boundary", summary = "F# callers/callees, implementations, hierarchy, C#-targeted ProjectReference semantics, and compatible-TFM dependency selection remain unsupported; indexed F# symbol-name search, exact-TFM F# ProjectReference definition closure, and selected-project compiler-bound references are supported" },
            ...
            semantic = new
            {
                ...
                note = "C# exact results are scoped to loaded candidate clusters. F# symbol_at and definition are compiler-checked from one proven physical root project/TFM through its exact-TFM F# ProjectReference closure; SDK projects are transitive by default while DisableTransitiveProjectReferences=true and legacy projects are direct-only, and child compiler errors retain source provenance and downgrade confidence like root errors. References use the closure but count only the selected root and remain a workspace lower bound because dependents are not scanned; successful results are exact only while disclosed partial reasons preserve selected-context authority; every error, authority loss, or unclassified partial reason is indexed. Restored package compile assets are snapshotted; C# dependencies and compatible-TFM selection fail closed without borrowing last-built DLLs; partial fields report bounded authority.",
    ...
    [Description("Reverse lookup: given a file + line (from a stack trace, build error, diff hunk, or grep hit), returns the smallest containing symbol and its enclosing chain plus owning projects. F# uses an FCS type-check context plus its exact-TFM F# ProjectReference closure; SDK projects are transitive by default, while DisableTransitiveProjectReferences=true and legacy projects are direct-only. Column is required when a line contains multiple symbol uses.")]
        ...
        string language, string operation, bool compileOwnedFSharp = false)
    {
        ...
        return Json.WithStringBudget(path, 4096, (boundedPath, pathTruncated) => new
        {
            ...
            detail = fsharpSemanticEligible
                ? "F# supports text, project-graph navigation, FCS outlines, position-based symbol_at, definition through the exact-TFM F# ProjectReference closure under the evaluated MSBuild transitivity policy, and selected-project references for compile-owned .fs/.fsi files. C# ProjectReference targets, compatible-TFM selection, implementations, callers, callees, and hierarchy remain unavailable."
                : language == "fsx"
