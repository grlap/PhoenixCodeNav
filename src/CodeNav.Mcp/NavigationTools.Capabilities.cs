        ...
        CodeNav.Core.Semantic.ProjectModelMode fsharpProjectModel = CodeNav.Core.Semantic.ProjectModelMode.Simple)
    {
        ...
        object envelope = new
        {
            ...
            features = new object[]
            {
                ...
                new { id = "index-raw-ordinal-symbol-batching", summary = "v0.12.24 build/delta C# symbols: cached exact-size 1..32 raw SQLite statements, ordinal binding; same output, no managed parameter-name lookup/allocation" },
                new { id = "index-raw-ordinal-file-batching", summary = "v0.12.35 file rows: client ids, cached raw ordinal SQLite statements; C# builds use exact 1..32 batches, delta/structural writes the same one-row path" },
                new { id = "index-deferred-secondary-index-build", summary = "v0.12.35 cold bulk-load retains primary/unique constraints; all nine query secondary indexes created in unpublished finalization transaction, measured cost reported" },
                ...
                new { id = "index-live-recovery-sidecar-publication-boundary", summary = "v0.12.68 Windows/Unix private construction preserves live SQLite recovery sidecars; re-inspect and reserve/remove current safe names only at post-drain publication" },
                ...
                new { id = "index-raw-ordinal-content-fts-batching", summary = "v0.12.37 exact-width raw ordinal file_contents/FTS5 batches; v0.12.38 keeps content batches, replaces cold FTS batches with one deferred rebuild; live content+FTS updates stay transactional" },
                new { id = "index-bounded-synchronous-csharp-build-handoff", summary = "v0.12.38 cold C# prepared sources: bounded synchronous queue to single writer; no sync-over-async ThreadPool starvation, full-width writer batches retained" },
                new { id = "index-build-request-dispatch-isolation", summary = "v0.12.69 startup/full rebuild and cold C# producer coordination: dedicated long-running execution lanes preserve server_capabilities dispatch under unrestricted parser fan-out, without changing parser concurrency, queue capacity, publication, or authority" },
                new { id = "index-deferred-fts-rebuild", summary = "v0.12.38 cold file_contents first, one external-content FTS5 rebuild in unpublished finalization; live incremental FTS maintenance stays transactional" },
                new { id = "index-size-prioritized-csharp-build-scheduling", summary = "v0.12.39 cold C# parsing: descending scanned bytes, ordinal-path tie-break; giant Roslyn parses overlap ordinary parsing/persistence, reducing final stragglers" },
                ...
                new { id = "search-symbol-type-relevance", summary = "v0.12.30 exact-name types rank above same-named members; neither class omitted" },
                ...
                new { id = "source-context-range-alias", summary = "v0.12.32 source_context: range aliases absent spans; conflicting values return bad_request" },
                ...
                new { id = "csharp-symbol-free-outline", summary = "v0.12.43 declaration-free C# files: outline/batch_outline retain symbols:[] and generated state" },
                ...
                new { id = "refresh-input-retry", summary = "v0.12.7 unavailable regular-source capture: whole-delta rollback; initial or event-driven serialized requests retry after 100/250/1000 ms; timer-initiated stale-index recovery uses separate paced cadence" },
                ...
                new { id = "refresh-recovery-self-heal", summary = "v0.12.27 unavailable-input stale recovery: autonomous complete sweeps, 5/10/30/60-second capped backoff; timer-initiated recovery sweeps make one capture attempt each, re-resolve pending Git baselines, remain honestly stale until success" },
                new { id = "nested-worktree-exclusion", summary = "schema43 parent scan/refresh/watcher exclude nested linked worktrees by registration, not name; explicit roots/submodules indexable" },
                new { id = "refresh-worker-failure", summary = "Pump fault: failed/refresh_worker_failed; log exception, retain DB/lease; no auto-restart" },
                new { id = "refresh-worker-admission-refusal", summary = "Dead pump seals queue; refresh_index auto/incremental/full: queued:false, restart daemon" },
                new { id = "refresh-callback-failure", summary = "v0.12.112 refresh_callback_failed: sweep clears; HEAD retries; refreshCallbackFailed JSONL" },
                new { id = "daemon-file-log", summary = "v0.12.113 .codenav/logs: daemon errors/crash hooks; 14-day retention" },
