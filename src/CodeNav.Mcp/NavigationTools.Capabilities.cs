        ...
        CodeNav.Core.Semantic.ProjectModelMode fsharpProjectModel = CodeNav.Core.Semantic.ProjectModelMode.Simple)
    {
        ...
        object envelope = new
        {
            ...
            features = new object[]
            {
                ...
                new { id = "semantic-cold-start-phase-timing", summary = "definition/references/implementations/type_hierarchy: fixed bounded timing.semanticColdStart for load/preparation/metadata-reference work/compilation/resolution on exact AND degraded C# semantic paths. Identical integer-ms snapshot in semanticOp telemetry; answers unchanged. Omitted for F# and calls ending before the C# pipeline" },
                ...
                new { id = "shared-daemon-session-recovery", summary = "v0.12.100 retryable startup failures reconnect the same MCP session to an existing daemon; no spawning or dispatched-call replay" },
                ...
                new { id = "assembly-ref-edges", summary = "legacy <Reference Include>+HintPath to an IN-WORKSPACE assembly creates an edge for dependents discovery, semantic cluster wiring and project_graph, including staged-build DLLs. Semantic binding prefers the SOURCE project (source-over-binary), enabling exact cross-project implementations/references. Assembly-name collisions (net-old/net-new pairs) retain all consumers through name-level edges in the graph and semantic workspace (schema v6+; recovery since v5). Every response carries meta.indexSchema" },
