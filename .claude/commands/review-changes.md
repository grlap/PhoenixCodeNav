...
4. Run `pwsh -NoProfile -File ./scripts/test-roslyn-mcp.ps1` against the pinned Roslyn and F# submodules. The harness bootstraps a missing reusable index through normal MCP startup and reuses an existing index; if a reused index disagrees with its fresh-index baseline canary (overview counts plus Roslyn reference-authority evidence), it performs one in-band full rebuild to capture before/after evidence, but that gate run still fails. Only a later run against the repaired matching fixture may pass.
5. Require the external MCP integration harness to exit successfully with zero failed cases. Missing submodules, mismatched external commits, any repair during the current run, a fresh rebuilt index that still disagrees with the baseline, or any harness failure stop the review gate; do not restore or update submodules implicitly.
